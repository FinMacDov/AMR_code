...
def movie_maker(mini_path, B, V, P, mini_sav_loc):
    ...
    V_str = str(np.int(V))
    P_str = str(np.int(P))
    file_sav_name = '/jet_B'+B_str+'_V'+V_str+'_P'+P_str+'_'
    render_view = []

    render_view.append(render_view_func())
    # loads data
    # create a new 'XML Unstructured Grid Reader'
    myPath = mini_path+'/jet_B'+B_str+'_V'+V_str+'P'+P_str
...
for ii in range(len(B)):
    for jj in range(len(V)):
        for kk in range(len(P)):
            movie_maker(mini_path=mini_path, B=B[ii], V=V[jj], P=P[kk], mini_sav_loc=sav_loc)
