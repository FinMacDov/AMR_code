...
!   open (unit = 13, file ="atmos_data/c7/fort/y.dat", status='old')
  ...
  do i=1,kmax  
   ...
   close(13)
    ...
    allocate(pbc(nghostcells))
    do ibc=nghostcells,1,-1
      na=floor((gzone-dx(1,refine_max_level)*(dble(nghostcells-ibc+1)-0.5d0))/dyk+0.5d0)
      res=gzone-dx(1,refine_max_level)*(dble(nghostcells-ibc+1)-0.5d0)-(dble(na)-0.5d0)*dyk
  ...
  subroutine initonegrid_usr(ixI^L,ixO^L,w,x)
    ...
    {do ix^DB=ixOmin^DB,ixOmax^DB\}
        na=floor((x(ix^D,1)-xprobmin1+gzone)/dya+0.5d0)
        res=x(ix^D,1)-xprobmin1+gzone-(dble(na)-0.5d0)*dya
...
!=> interpolation to obtain rho & p of HSE
    do ix1=ixOmin1,ixOmax1
        na=floor((x(ix^D,1)-xprobmin1+gzone)/dyk+0.5d0)
        res=x(ix^D,1)-xprobmin1+gzone-(dble(na)-0.5d0)*dyk
  ...
  subroutine specialbound_usr(qt,ixI^L,ixO^L,iB,w,x)
    ...
    case(3)
      ...
      do idir=1,ndir
        w(ixO^S,mom(idir)) =-w(ixOmax1+nghostcells:ixOmax1+1:-1,mom(idir))&
                   /w(ixOmax1+nghostcells:ixOmax1+1:-1,rho_)
      end do
    ...
    case(4)
      ...
      tmp=0.d0
      do ix1=ixOmin1,ixOmax1
        tmp(ixOmin2-1^%2ixO^S)=tmp(ixOmin1-1^%2ixO^S)+0.5d0*&
            (ggrid(ix1^%2ixO^S)+ggrid(ix1-1^%2ixO^S))*invT(ixOmin1-1^%2ixO^S)
        w(ix1^%2ixO^S,p_)=pth(ixOmin2-1^%2ixO^S)*dexp(tmp(ixOmin2-1^%2ixO^S)*dxlevel(1))
        w(ix1^%2ixO^S,rho_)=w(ix1^%2ixO^S,p_)*invT(ixOmin1-1^%2ixO^S)
      enddo

      !> fixed zero velocity
      do idir=1,ndir
        w(ixO^S,mom(idir)) =-w(ixOmin1-1:ixOmin1-nghostcells:-1,mom(idir))&
                     /w(ixOmin1-1:ixOmin1-nghostcells:-1,rho_)
      end do
      call phys_to_conserved(ixI^L,ixO^L,w,x)
    case default
  ...
  subroutine getggrav(ggrid,ixI^L,ixO^L,x)
    ...
    ggrid(ixO^S)=usr_grav*(SRadius/(SRadius+x(ixO^S,1)))**2
  end subroutine
